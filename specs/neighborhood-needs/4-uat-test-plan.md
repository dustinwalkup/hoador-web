...
| NN-06 | P2  | Provider (push opted in) | Opt into push for the category, then have a new need posted                                                     | A push is delivered in addition to in-app                                                       | R12.3             | ⏸ Retest on staging — BUG FOUND & FIXED: fanOutNewNeed hard-coded sendPush:false, so push never fired regardless of opt-in. Removed it (service.ts) so shouldSendPush() gates it. Redeploy staging → opted-in User B should now receive push.                                                        |
| NN-07 | P1  | Out-of-network user      | When a need is posted outside your network                                                                      | You receive **no** new-need notification                                                        | R12.2             | ✅ Pass — 2026-07-14 (out-of-network gets nothing)                                                                                                                                                                                                                                                   |
