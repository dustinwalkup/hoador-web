...
| DET-05 | P1  | Out-of-network user  | Open a need detail URL you cannot see | **404 / not found** (creator and admins are the only exceptions)                                                                                  | R6.2 | ✅ Pass (clean data). ⚠️→✅ Staging leak FIXED in code: detail route now enforces SYMMETRIC visibility (viewer AND creator must be visible) — page.tsx + api/needs/[id]/route.ts, reusing communityDAL.isVisibleInCommunity. Regression test added. Also needs the staging data reconcile. Retest after redeploy. |
...
| NN-05 | P1  | Provider                 | **Mute** the Neighborhood Needs category in notification preferences, then have the requester post another need | You receive **nothing** for it                                                                  | R12.4             | ❌ FAIL — no in-app mute exists. Prefs table has only email/push columns; sendNotification always creates the in-app row (send-notification.ts:71). Muting still shows in-app → breaks central promise #5 'can be muted'. DECISION NEEDED: add per-category in-app/enabled toggle, or rescope R12.4.                                                        |
...
| NN-07 | P1  | Out-of-network user      | When a need is posted outside your network                                                                      | You receive **no** new-need notification                                                        | R12.2             | ✅ Pass (clean data). ⚠️→✅ Staging leak FIXED in code: fanOutNewNeed now skips entirely when the creator isn't visible in the community (symmetric). Regression test added. Also needs staging data reconcile + (recommended) updateCommunity guard. Retest after redeploy.                                                                                |
