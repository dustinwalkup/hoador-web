...
type NewToolImage = InferInsertModel<typeof toolImages>;
...
// Tool templates based on available images
const toolTemplates = [
  {
    image: "car-jack.jpg",
...
async function main() {
  ...
  for (let i = 0; i < totalTools; i++) {
    ...
    // Create tool image entry using public URL
  ...
  console.log(`📦 Inserting ${seedTools.length} tools...`);
  await db.insert(tools).values(seedTools);

  console.log(`🖼️ Inserting ${seedImages.length} tool images...`);
  await db.insert(toolImages).values(seedImages);
