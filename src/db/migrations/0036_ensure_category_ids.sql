-- Plain SQL (no PL/pgSQL) to ensure correct category IDs exist.
-- Step order matters: update FKs first, then delete old rows, then insert correct rows.

-- ─── Listing categories ──────────────────────────────────────────────────────

-- 1. For any listing pointing to a wrong-ID row, update it to the correct ID
--    (only fires when the wrong-ID row exists alongside the correct name)
...
-- 2. Delete wrong-ID category rows (FKs already moved above)
...
-- 3. Insert any missing categories (skips rows that already exist)
...
-- 4. NULL out any listings still pointing to a category that no longer exists
UPDATE listings
SET category_id = NULL
WHERE category_id IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM listing_categories lc WHERE lc.id = listings.category_id);

-- ─── Service listing categories ──────────────────────────────────────────────
-- Uses a separate table so it only runs if service_listing_categories exists.
-- Drizzle migrations are transactional — if the table doesn't exist this will
-- error, so we guard with a DO block only for the existence check, then use
-- plain INSERT for the actual data work.
...
BEGIN
  ...
  -- Migrate service listings off wrong-ID rows
