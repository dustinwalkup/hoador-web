 ...
 * to the caller's own `stripeCustomerId`. Mocking the session module (not
 * route-helpers) keeps the real `getAuthenticatedUserResponse` — including
 * its actual 401 mapping — under test.
 */

const mockAttachPaymentMethod = vi.fn();
vi.mock("@/services/stripe/payment-method", () => ({
  attachPaymentMethod: (...a: unknown[]) => mockAttachPaymentMethod(...a),
}));
...
const req = (body: unknown = { paymentMethodId: "pm_1" }) =>
  new NextRequest("http://localhost/api/stripe/attach-payment-method", {
    method: "POST",
...
describe("POST /api/stripe/attach-payment-method", () => {
  ...
  it("401s without a session and never attaches", async () => {
  ...
  it("400s a missing paymentMethodId, without attaching", async () => {
    const res = await POST(req({}));
  ...
  it("200s and attaches with the caller's own customer/user ids", async () => {
    ...
    expect(body).toEqual({
      success: true,
      paymentMethod: { id: "pm_1", object: "payment_method" },
    });
