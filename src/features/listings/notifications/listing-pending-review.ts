...
/**
 * Notify all active admin/superadmin users that a rental listing needs review.
 */
export async function sendRentalListingPendingAdminNotification(listing: {
  id: string;
  name: string;
  ownerId: string;
}): Promise<void> {
  const baseUrl = appBaseUrl();
  const linkUrl = `${baseUrl}/admin/dashboard/listings/review`;
  const staff = await userDAL.getStaffNotificationRecipients();
  const owner = await userDAL.getUserById(listing.ownerId);
  const ownerLabel =
  ...
  await Promise.all(
    staff.map((admin) =>
      sendNotification({
        ...
        console.error(
          `Failed to send listing pending admin notification for ${admin.id}:`,
          err,
        ),
      ),
    ),
