...
interface InactiveServiceListingsProps {
  filters: ServiceListingFilters;
}

export function InactiveServiceListings({
  filters,
}: InactiveServiceListingsProps) {
  const { data, isLoading, error, refetch } = useMyServiceListings("inactive");
      ...
      {listings.length > 0 ? (
        ...
        <div className="col-span-full py-8 text-center">
          <div className="bg-muted mb-4 inline-flex rounded-full p-3">
            <PauseCircle className="text-muted-foreground h-6 w-6" />
          </div>
          ...
          <p className="text-muted-foreground text-sm">
            {hasFilters
              ...
              : "Listings you've deactivated will appear here"}
          </p>
        </div>
      )}
