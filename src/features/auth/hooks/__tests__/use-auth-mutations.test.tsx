...
describe("useAdminLogin", () => {
  ...
  it("should login admin successfully", async () => {
    ...
    await waitFor(() => {
      expect(mockFetch).toHaveBeenCalledWith("/api/auth/admin-login", {
        method: "POST",
        body: expect.any(FormData),
      });
      expect(toast.success).toHaveBeenCalledWith(
  ...
  it("should handle non-admin user attempting admin login", async () => {
    const errorResponse = { error: "Admin privileges required" };
    mockFetch.mockResolvedValue({
      ok: false,
      status: 403,
      json: async () => errorResponse,
  ...
  it("should not redirect after admin login", async () => {
    const mockResponse = {
      success: true,
    };
